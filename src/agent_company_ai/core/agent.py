...
class Agent:
    ...
    async def think(self, task: Task, max_iterations: int = 15) -> str:
        ...
        for iteration in range(max_iterations):
            self._assert_tool_chain(messages)
            try:
            ...
            for tc in response.tool_calls:
                ...
                # Always append the tool result BEFORE the terminal check so
                # the assistant tool_calls message is never left unanswered.
                messages.append(LLMMessage(
                    role="tool",
                    content=tool_result,
                    tool_call_id=tc.id,
    ...
    def _repair_conversation(self) -> None:
        ...
        while i < n:
            ...
            if len(matched) == len(expected_ids) and set(matched) == set(expected_ids):
    ...
    @staticmethod
    def _assert_tool_chain(messages: list[LLMMessage]) -> None:
        """Fail loudly if the message chain violates the tool-call contract.

        OpenAI-compatible APIs reject conversations where an assistant
        message carrying tool_calls is not immediately followed by tool
        responses for every declared id (non-retryable 400). This guard
        converts that into a precise assertion at the call site so a
        regression is caught instantly instead of surfacing as an opaque
        API error.
        """
        if not messages:
            return
        for i, msg in enumerate(messages):
            if not (msg.role == "assistant" and msg.tool_calls):
                continue
            declared = {tc["id"] for tc in msg.tool_calls if tc.get("id")}
            j = i + 1
            ...
            if missing:
                raise RuntimeError(
                    f"Invalid message chain: assistant tool_calls {missing} "
                    f"have no tool response before the next request."
                )

    async def _execute_chat_tool(self, tc) -> str:
        """Execute one tool call in chat mode and return its result string.

        Executes the underlying tool directly (no nested LLM call) so the
        caller's assistant -> tool message pairing stays intact. A nested
        provider.complete() here would send a conversation ending in an
        unanswered assistant tool_calls message, which OpenAI-compatible
        APIs reject with 'insufficient tool messages following tool_calls'.
        """
