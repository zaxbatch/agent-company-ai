...
def _groove_of(genre):
    """Look up a genre's groove class from the single source of truth."""
    import importlib.util
    spec = importlib.util.spec_from_file_location("vs", str(ROOT / "scripts" / "verify_song.py"))
    m = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(m)
    return m.GENRES.get(genre, {}).get("groove", "?")
...
for t in man["tracks"]:
    ...
    # NOTE: genre is deliberately NOT part of this key. Including it made a
    # collision impossible and the check vacuous -- two tracks in different
    # genres could share a groove, tempo and brightness and still pass.
    key = (round(bpm / 10) * 10, int(cent // 250), _groove_of(t["genre"]))
    fp[t["n"]] = key
