#!/usr/bin/env python3
"""post_cooler_dub.py — encode, upload and post "Frostline Dub" to SnowSnakes.

Follows the established pipeline (build_disco_song.py / build_boombap_song.py):
  wav -> mp3 (libmp3lame 192k) -> Cloudinary (unsigned preset) -> POST /api/songs

Posted as an existing persona account (rotating authorship, like every other
release) using the credentials in .snowsnakes_real_users.json.
"""
...
WAV = SONGS / "wav" / "frostline-dub.wav"
MP3 = SONGS / "audio" / "frostline-dub.mp3"
COVER = SONGS / "art" / "frostline-dub.png"
USERS = ROOT / ".snowsnakes_real_users.json"

TITLE = "Frostline Dub"
AUTHOR_ID = 76          # kai_torres -- has not released a song yet
...
def encode():
    ...
    peak = [l.split("max_volume:")[1].strip() for l in out.splitlines() if "max_volume" in l]
    print(f"  mp3 {MP3.stat().st_size//1024} KB  max_volume={peak[0] if peak else '?'}")


def upload(path, rtype):
    b = "----zdotdub" + uuid.uuid4().hex
    body = b"".join([
...
def main():
    ...
    ap.add_argument("--post", action="store_true", help="actually post (default: build+upload only)")
    args = ap.parse_args()
    ...
    tok = res.get("token")
    if not tok:
        raise SystemExit(f"FATAL: login failed for {author['username']} ({st})")
    ...
    print("\nVERIFY on live board:")
    print("   found:", json.dumps(row, indent=1)[:400] if row else "NOT FOUND")
    (ROOT / "evidence").mkdir(exist_ok=True)
    (ROOT / "content" / "snowsnakes" / "songs" / "frostline-dub-post.json").write_text(
