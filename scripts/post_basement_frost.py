#!/usr/bin/env python3
"""post_basement_frost.py — encode, upload and post "Basement Frost" to SnowSnakes.

Same pipeline as the rest of the catalogue: wav -> mp3 (libmp3lame 192k) ->
Cloudinary (unsigned preset) -> POST /api/songs, as a rotating persona account.
"""
...
WAV = SONGS / "wav" / "basement-frost.wav"
MP3 = SONGS / "audio" / "basement-frost.mp3"
COVER = SONGS / "art" / "basement-frost.png"
USERS = ROOT / ".snowsnakes_real_users.json"

TITLE = "Basement Frost"
AUTHOR_ID = 79          # ivy_chen -- has not released a song yet
...
def main():
    ...
    print("\nVERIFY on live board:", json.dumps(row, indent=1)[:380] if row else "NOT FOUND")
    (SONGS / "basement-frost-post.json").write_text(
