...
| zdot-dummy.com    | **NOT REGISTERED** (verified available 2026-08-30 via Verisign RDAP 404 + DNS NXDOMAIN) | — (registration pending, recommended: Hostinger hPanel — existing account+API+billing) | — (auto parking once registered) | **NONE by design** (non-deliverable syntax domain — see MAIL DECISION) | TEST/QA/SEED/persona domain per BossLady policy. Registration cost ~$10-16 first yr, renew ~$16-22/yr. |
...
## MAIL DECISION — zdot-dummy.com (NinjaNerd/CTO, 2026-08-30)
...
- Anti-abuse: once registered, add TXT `v=spf1 -all` (never authorized to send) and `v=DMARC1; p=reject;` so spoofed @zdot-dummy.com mail is rejected by receivers. NO MX = no catch-all to harvest.
- **UPGRADE PATH (documented, ~15 min)**: if/when any app adds verification/reset email flows for test accounts, add via Hostinger hPanel: MX 10 mx1.hostinger.com / MX 5 mx2.hostinger.com + create one catch-all mailbox (e.g. catchall@zdot-dummy.com). DNS and mail are already Hostinger — no registrar change needed.

## REGISTRATION PLAN — zdot-dummy.com
1. Buy via Hostinger hPanel (account + API token + billing already on file) OR Squarespace Domains if BossLady prefers one bill with zdotllc.com. Hostinger recommended: same DNS/mail console, one provider.
2. Cost: .com ~$10-16 first year (promo-dependent), renewal ~$16-22/yr. WHOIS privacy free.
3. Post-purchase: confirm ownership (RDAP shows registrant), add SPF -all + DMARC reject TXT, keep NO MX.
4. Ownership should be registered to Z-Dot LLC (registrant org) for asset tracking.
...
## COSTS (annual, est.)
- zdot-dummy.com registration: ~$10-16 first year + ~$16-22 renewal. NO mailbox plan (decision above) = $0/mo mail.
